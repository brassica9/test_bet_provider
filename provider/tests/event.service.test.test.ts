import {
  ...
  getEvents,
  statusEvent,
} from "../src/modules/provider/event.service";
import prisma from "../../utils/prisma";
import { notifyBetPlatform } from "../src/utils/notifyBetPlatform";
...
jest.mock("../src/utils/notifyBetPlatform", () => ({
    ...
    expect(result).toEqual([
      {
        ...
        coefficient: 1.5,
        deadline: Math.floor(
          new Date("2024-09-30T10:52:00.000Z").getTime() / 1000
        ),
        status: "pending",
        createdAt: expect.any(Date),
      },
      {
        ...
        coefficient: 2.0,
        deadline: Math.floor(
          new Date("2024-09-30T11:00:00.000Z").getTime() / 1000
        ),
        status: "pending",
        createdAt: expect.any(Date),
      },
